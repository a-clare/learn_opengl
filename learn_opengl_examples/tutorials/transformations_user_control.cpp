...
int use_color;
float user_x_translate = 0.0;
float user_y_translate = 0.0;
int pause = 1;
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods);

int main(int argc, char* argv[]) {
  std::cout << "Press 1 to toggle color" << std::endl;
  std::cout << "Press left/right/up/down to move" << std::endl;
  std::cout << "Press space to stop rotation" << std::endl;
  if (argc < 2) {
  ...
  */
  ...
  while (!display.Shutdown()) {
    ...
    transform = glm::translate(transform, glm::vec3(user_x_translate, user_y_translate, 0.0f));
